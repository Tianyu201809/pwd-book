...
全局快捷键 `Alt+Shift+O` 在启动与 `settings:update` 时注册，退出前注销。快捷键本身不依赖 `clipboardEnabled`；关闭「剪切板历史」后仍可打开小窗并手动新建文本，但不再轮询系统剪切板。
...
| 入口 | 条件 |
|------|------|
| 全局快捷键 `Alt+Shift+O` | 始终注册；锁定态引导解锁 |
| 标题栏剪切板按钮 | 已解锁且非详情小窗口 |
| 侧栏 **工具箱 → 剪切板** | 已解锁 |
...
- 未解锁不轮询、不展示列表。
- 关闭「剪切板历史」后启动时不监听系统剪切板；已打开的小窗仍可手动新建文本。
