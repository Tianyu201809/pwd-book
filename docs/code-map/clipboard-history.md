...
小窗内键盘（**v1.33.0**）：`↑`/`↓` 选择条目，`Enter` 复制，`Ctrl+Enter` / `Meta+Enter` 预览，`Esc` 关闭。**v1.34.0** 开启**快捷模式**后，`Enter` 复制成功即关闭小窗（点击复制按钮不关）。`Delete` / `Backspace` 删除当前选中条目（输入框内除外），并选中相邻下一条。复制成功经小窗 `ToastHost` 提示。
