...
**v1.24.0** 交互式新手引导：标题栏 **产品学习**（学士帽）→ 引导中心 → 分步聚光灯/遮罩高亮。
...
| `organize` | 分类与标签 | 分类、标签筛选、新建分类、工具区 |
| `entries` | 条目管理 | 搜索、排序/布局、新建、详情侧栏 |
| `tools` | 实用工具 | 展开工具区、随机密码、密码健康、回收站 |
| `titlebar` | 标题栏快捷 | 换肤、锁定、置顶、学习入口 |
| `settings` | 设置中心 | 侧栏设置入口 → 设置 Tab → 安全 / 数据 |
...
VaultSidebar                 # 监听 TOUR_PREPARE_EVENT 展开工具区/标签筛选
```
...
| `sidebar-categories` / `sidebar-tag-filter` / `sidebar-new-category` / `sidebar-utilities` | `VaultSidebar.vue` |
...
| 动作 | 说明 |
|------|------|
| `expand-utilities` | `VaultSidebar` 展开「工具与设置」 |
| `expand-tag-filter` | 展开标签筛选 |
...
侧栏展开经 `window` 事件 `pwdbook-tour-prepare`（`TOUR_PREPARE_EVENT`）派发。
