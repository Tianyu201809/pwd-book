...
**v1.24.0** 交互式新手引导：标题栏 **产品学习**（学士帽）→ 引导中心 → 分步聚光灯/遮罩高亮。**v1.25.0** 适配侧栏底部图标栏与列表工具栏标签筛选的新 UI 布局。**v1.33.0** 设置中心对齐新分区（剪切板 / 浏览器 / 悬浮条 / 回收站）；工具箱与标题栏路径补上剪切板入口。**v1.39.0** 设置中心补上便签分区。
...
| `settings` | 设置中心 | 侧栏 **设置** → 分区导航 → 安全 / 剪切板 / 便签 / 浏览器 / 悬浮条 / 回收站 / 数据 |
