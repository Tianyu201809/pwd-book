...
import BrowserExtensionGuideVisual from '@/components/browser/BrowserExtensionGuideVisual.vue'
...
const props = defineProps<{
  ...
  extensionsPageHint?: string
}>()

const emit = defineEmits<{
  'update:extensionId': [value: string]
  openExtensions: []
  register: []
}>()
...
const steps = computed(() => {
  const raw = tm('settings.browserFillGuide.steps') as Array<{
    title: string
    desc: string
    tip?: string
  }>
...
function onOpenExtensionsClick(): void {
  emit('openExtensions')
}
  ...
  >
    <template #title>
      <div class="guide-modal-title">
        ...
        <span>
          ...
          <small>{{ t('settings.browserFillGuide.subtitle') }}</small>
        </span>
      </div>
    </template>
    ...
    <div ref="guideRootRef" class="browser-extension-guide guide-root">
      ...
      <p class="guide-step-label">
        {{ t('settings.browserFillGuide.stepOf', { current: currentStep + 1, total: STEP_COUNT }) }}
      </p>
      ...
      <div v-if="stepMeta" class="guide-step-copy">
        ...
        <div v-if="currentStep === 3" class="guide-step-register">
          ...
          <UiInput
            ...
            allow-clear
            @update:model-value="emit('update:extensionId', $event)"
          />
          <div class="guide-step-actions">
            <UiButton
              variant="primary"
              size="small"
              :disabled="!extensionId.trim() || registerLoading"
...
<style>
/* 经典主题：内容区滚动，底部按钮固定，避免 hover 位移触发滚动条 */
.modal-overlay:has(.browser-extension-guide) .modal-card {
  display: flex;
  flex-direction: column;
...
.modal-overlay:has(.browser-extension-guide) .modal-body {
  flex: 1 1 auto;
  min-height: 0;
  overflow-x: hidden;
  overflow-y: auto;
...
.modal-overlay:has(.browser-extension-guide) .modal-footer {
  flex-shrink: 0;
  border-top: 1px solid var(--border-default);
}

/* 动森主题：仅内容区滚动，footer 与 animal-modal__content 分离 */
[data-skin='animalIsland'] .animal-modal:has(.browser-extension-guide) {
  max-height: min(90vh, 860px);
}

[data-skin='animalIsland'] .animal-modal:has(.browser-extension-guide) .animal-modal__content {
  flex: 1 1 auto;
  min-height: 0;
  overflow-x: hidden;
}

[data-skin='animalIsland'] .animal-modal:has(.browser-extension-guide) .animal-modal__footer {
  flex-shrink: 0;
  width: 100%;
  padding-top: 12px;
  border-top: 1px solid var(--border-default);
