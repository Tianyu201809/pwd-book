...
应用提供「数字保险库」与 **Animal Island** 两套视觉皮肤，标题栏可**快捷换肤**；**v1.20.0** 主窗口标题栏支持 **窗口置顶** 图钉；**v1.21.0** **设置 → 安全** 支持 **开机自动启动**（默认关闭；**v1.23.0** 修复 Windows 含空格安装路径）；**v1.22.0** 条目支持 **附件**、**自定义字段** 与 **列表/方块** 布局切换；**v1.23.0** 托盘菜单可直达 **设置**、数据库损坏时自动隔离备份并创建空库；**v1.24.0** 标题栏 **产品学习** 提供 6 条交互式引导路径；**v1.25.0** 侧栏底部改为 **工具箱 / 管理 / 回收站 / 设置** 图标栏，**标签筛选** 迁至列表工具栏；**v1.26.0** 快捷条可配置显示条数、无目标条目可定位主窗口、列表可滚动，并修复 macOS 无法退出；**v1.30.0** 条目可选用本地**预设品牌图标**，填写已知网址且未选图标时自动匹配；**v1.31.0** 复制字段有 Toast 反馈，标题首字/字母头像对比度提高；**v1.32.0** 提供**剪切板历史**独立小窗口（`Alt+Shift+O`，默不监听系统剪切板）；**v1.33.0** 设置拆为安全 / 剪切板 / 浏览器 / 悬浮条 / 回收站等分区，并补齐剪切板使用向导与产品探索；**v1.34.0** 剪切板小窗默认不固定，并可开快捷模式（回车复制后关窗）；**v1.36.0** 快捷条支持右侧详情卡片、字段快捷复制与 TOTP 动态验证码；**v1.38.0** 提供加密**轻量便签**（管理窗口、桌面便签、便签本）；**v1.39.0** 增加便签管理全局快捷键，并在设置中单独分区开关。支持中英文界面；默认窗口 1200×760，自定义标题栏。官方打包支持 **Windows**（NSIS）、**macOS**（DMG，x64 / arm64）与 **Linux**（AppImage）；推送 `v*` tag 可经 GitHub Actions 发布 Release。
...
| **条目字段** | 标题、网址、用户名、密码、备注、标签、**显示图标**（**v1.30.0** 起含本地品牌 Logo）、所属分类、**本地程序路径**（可选）、**TOTP 两步验证密钥**（**v1.12.0**，选填；实时 6 位验证码，30 秒刷新，可复制；**v1.13.0** 起编辑时可显示/隐藏密钥）、**自定义字段**（**v1.22.0**，name/value，每条条目最多 20 个）、**附件**（**v1.22.0**，加密存于本地，每条条目最多 10 个、单文件 ≤ 5 MB；可打开、另存为、删除） |
...
| **回收站** | 侧栏底部 **回收站** 图标（有条目时显示角标）；删除的条目保留可配置天数（**设置 → 回收站**，默认 30 天），过期自动彻底删除；支持单条/全部还原、单条/清空彻底删除 |
| **侧栏底部栏**（**v1.25.0**） | 四图标：**工具箱**（随机密码、密码健康、**v1.32.0** 剪切板、**v1.38.0** 便签）、**管理**（分类/标签）、**回收站**、**设置**；悬停显示名称，工具箱与管理弹出子菜单；锁定改由标题栏快捷入口 |
...
从侧栏进入 **设置**。**v1.33.0** 起分为：**安全 / 剪切板 / 浏览器 / 悬浮条 / 回收站 / 外观 / 数据 / 关于**；**v1.39.0** 增加 **便签** 分区。
...
| 剪贴板自动清除 | 复制密码等文本后，若剪贴板内容未被改写，约 30 秒后清空（与剪切板历史无关） |
...
| 项 | 说明 |
|----|------|
| **剪切板历史** | 开关（默认关闭）；开启后自动捕获复制的文本与图片。**v1.43.0** 唤出快捷键可录制，默认 `Alt+Shift+O` |
| 默认清理周期 | 30 秒 / 5 / 15 / 30 分钟 / 永久（默认 5 分钟）；固定记录除外 |
| 历史条数上限 | 20 / 50 / 100 / 200（默认 50）；超过时先删最旧未固定记录 |
| 数据持久化 | 可选重启后保留；默认仅本次会话 |
| **使用向导** | 分步弹窗：开启、唤出小窗、捕获、取回、存放 |
| **快捷模式**（**v1.34.0**） | 在剪切板小窗工具栏开关；开启后回车复制并关闭窗口（设置项 `clipboardQuickMode`，默认关闭） |

#### 便签（**v1.39.0**）

| 项 | 说明 |
|----|------|
| 快捷键打开便签 | 开关（默认开启）；**v1.43.0** 可录制组合键，默认 `Alt+Shift+N` 打开或收起便签管理窗口。锁定时先唤起主窗口以便解锁 |

#### 浏览器（**v1.33.0**）

| 项 | 说明 |
|----|------|
| **浏览器自动填充** | 本机桥接 + Chrome/Edge 扩展，保险库已解锁时按域名填充。**安装向导**分步完成部署。见 [浏览器扩展](#浏览器扩展chrome--edge) |

#### 悬浮条（**v1.33.0**）

| 项 | 说明 |
|----|------|
| 快捷搜索条 | 开关 +「打开快捷条」；**v1.43.0** 可录制组合键，默认 `Alt+Shift+P`；显示条数 5–20（默认 5） |
| 快捷键唤起主窗口 | 独立开关，位于悬浮条开关下方（**v1.43.0**）；可录制组合键，默认 `Alt+Shift+M` |
...
从侧栏底部 **工具箱 → 剪切板**、标题栏剪切板按钮、托盘右键「打开剪切板」（**v1.43.0**），或全局快捷键（默认 **`Alt+Shift+O`**，**v1.43.0** 可在 **设置 → 剪切板** 录制更换）打开独立小窗口（须已解锁；锁定态会先唤起主窗口）。
...
- **v1.43.0** 唤出快捷键可在 **设置 → 剪切板** 录制更换，默认仍是 `Alt+Shift+O`
...
从侧栏底部 **工具箱 → 便签**，或全局快捷键（默认 **`Alt+Shift+N`**，**v1.43.0** 可在 **设置 → 便签** 录制更换）打开独立管理窗口（须已解锁；已打开再按一次收起；锁定态会先唤起主窗口）。可在 **设置 → 便签** 关闭该快捷键。也可从托盘右键「打开便签」进入（**v1.43.0**）。

- **v1.43.0** 右键便签本可在该本中新建便签；桌面便签标题栏可最小化到任务栏
...
- 便签本右键可新建便签（**v1.43.0**），也可重命名或删除；删除时可把便签移到默认本或一并进回收站
- 标题与正文经会话密钥加密，随备份和同步走；桌面位置与是否显示只留本机。锁定时全部隐藏。详见 [docs/code-map/sticky-notes.md](./docs/code-map/sticky-notes.md)
...
#### 图标选择（v1.11.0 / v1.30.0）

分类与条目的 **显示图标** 可在图标选择器中挑选：**图形图标** 或 **A–Z 字母**（分 Tab）。**v1.30.0** 起，**条目**选择器另有 **品牌** 页：随安装包预置的常用网站/软件彩色 Logo（本地文件，不访问网络）；填写已知网址且尚未选图标时自动带上对应品牌图，已选图标不覆盖。分类选择器不含品牌页。侧栏与设置页导航项使用彩色徽章风格。
...
| 实用工具 | 随机密码、密码健康、剪切板、便签、回收站 |
| 标题栏快捷 | 换肤、锁定、剪切板、置顶、学习入口 |
| 设置中心 | 设置分区导航，以及安全 / 剪切板 / 便签 / 浏览器 / 悬浮条 / 回收站 / 数据 |
...
3. **手动安装** — Chrome/Edge 打开扩展管理页，开启开发者模式，**加载已解压的扩展程序**。安装版先点 **「打开 resources 目录」**，再选择其中的 `extension` 文件夹；开发版选择仓库中的 `extension/`。
...
- **系统托盘** — 标题栏「−」或关闭流程可选「最小化到托盘」；托盘图标点击可重新显示窗口；托盘菜单可打开 **快捷搜索**（**v1.12.0** 起菜单文案随界面语言切换）、**剪切板**、**便签**（**v1.43.0**）与 **设置**（**v1.23.0**；锁定态解锁后自动进入设置页）
- **悬浮快捷搜索条** — 置顶细长搜索窗（默认 `Alt+Shift+P`）；`↑`/`↓` 选择、`Enter`：优先启动**本地程序**，否则**打开网址**，均未填写则打开主窗口并定位（**v1.26.0**）；无输入时显示 **最近打开**（条数可配，默认 5、最大 20，独立存储，清空后不会从历史 `last_used_at` 自动补满；**v1.26.0** 结果区固定高度可滚动）；锁定状态下唤起会引导先解锁主窗口
- **剪切板历史小窗口**（**v1.32.0**） — 默认 `Alt+Shift+O`；置顶、不占任务栏；**v1.34.0** 默认不固定，未固定时点到其他程序会收起；可开快捷模式（回车复制后关窗）；用 Esc、关闭按钮或再次快捷键也可收起；锁定态唤起会引导先解锁。见 [剪切板历史](#剪切板历史-v1320)
- **桌面便签**（**v1.38.0**） — 从管理窗弹出；无系统边框，可置顶；**v1.43.0** 标题栏可最小化到任务栏；关闭只隐藏；锁定保险库时全部隐藏，解锁后恢复（含已最小化的窗）。管理窗快捷键默认 **`Alt+Shift+N`**（**v1.39.0**，**v1.43.0** 可改，可在设置关闭）。见 [便签](#便签-v1380)
- **快捷键唤起主窗口** — 全局快捷键（默认 `Alt+Shift+M`，**v1.43.0** 可在 **设置 → 悬浮条** 录制更换）显示并聚焦主窗口；开关在悬浮条开关下方单独配置
- **关闭行为** — 设置 → 安全：每次询问、默认最小化到托盘、或直接退出；标题栏关闭对话框可勾选「记住选择」；**v1.26.0** 修复 macOS Cmd+Q / Dock 退出被拦截的问题
...
- **剪贴板**：可选在复制后 N 秒清空剪贴板（若内容未被用户改写）。**v1.32.0** 剪切板历史默认关闭；开启后明文只存在本机渲染进程存储，不进保险库、不同步。
- **便签**（**v1.38.0**）：标题、正文和便签本名称与密码条目一样经会话密钥加密；锁定后窗口隐藏且内存草稿不得写入明文临时文件。
...
| 类别 | 内容 |
|------|------|
| 全局快捷键 | 便签、剪切板、悬浮窗、主窗口的启动组合键可录制更换，默认键不变 |
| 托盘 | 右键可打开剪切板和便签 |
| 桌面便签 | 小窗可最小化到任务栏 |
| 便签本 | 右键便签本可在该本中新建便签 |
...
| 类别 | 内容 |
|------|------|
| 便签管理 | 便签本栏与列表栏可调宽，宽度保存在本机 |
| 便签本 | 列表条目右键移动到其他便签本，移动前保存草稿；工具栏改为只读名称 |

### v1.39.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1390---2026-09-20)**。摘要：

| 类别 | 内容 |
|------|------|
| 快捷键 | `Alt+Shift+N` 打开或收起便签管理窗口；锁定时先唤起主窗口 |
| 设置 | 新增 **便签** 分区，可开关该快捷键 |

### v1.38.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1380---2026-09-20)**。摘要：

| 类别 | 内容 |
|------|------|
| 便签 | 工具箱入口、管理窗口、桌面便签、便签本、颜色、收藏、回收站 |
| 编辑 | 文本/待办混排；锁定隐藏，解锁恢复桌面便签 |
| 数据 | 加密存储；导出载荷 version 3；同步含便签，窗口几何不同步 |

### v1.34.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1340---2026-09-06)**。摘要：

| 类别 | 内容 |
|------|------|
| 剪切板 | 小窗第一次打开默认不固定 |
| 快捷模式 | 回车复制成功后自动关闭小窗（点击复制不关） |

### v1.33.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1330---2026-09-05)**。摘要：

| 类别 | 内容 |
|------|------|
| 设置 | 拆为安全 / 剪切板 / 浏览器 / 悬浮条 / 回收站 / 外观 / 数据 / 关于 |
| 剪切板 | 条数上限；使用向导；小窗失焦不隐藏，键盘选择/复制/预览 |
| 引导 | 产品探索对齐新分区与剪切板入口 |

### v1.32.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1320---2026-09-04)**。摘要：

| 类别 | 内容 |
|------|------|
| 剪切板 | 独立小窗口记录文本/图片；`Alt+Shift+O`；标题栏与工具箱入口 |
| 设置 | **剪切板历史**开关（默认关）、默认清理周期、可选持久化 |
| 行为 | 未固定失焦隐藏；锁定隐藏；历史不进保险库 |

### v1.31.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1310---2026-09-03)**。摘要：

| 类别 | 内容 |
|------|------|
| 反馈 | 详情页复制字段后 Toast（约 3 秒） |
| 可读性 | 标题首字 / 字母头像改为深色字 + 浅底 |

### v1.30.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1300---2026-08-30)**。摘要：

| 类别 | 内容 |
|------|------|
| 图标 | 条目选择器新增 **品牌** 页；本地预置常用网站/软件彩色 Logo（离线、单张 ≤ 500KB） |
| 自动匹配 | 编辑时填写已知网址且图标为空，自动带上对应品牌图；已选不覆盖 |
| 展示 | 列表 / 方块 / 回收站 / 快捷条 / 详情统一渲染；缺失则退回标题首字 |

### v1.26.0

完整变更列表见 **[CHANGELOG.md](./CHANGELOG.md#1260---2026-07-12)**。摘要：

| 类别 | 内容 |
|------|------|
| 快捷条 | 本地程序 / 打开网址 / 无目标时定位主窗口；显示条数可配（5–20）；结果列表固定高度滚动 |
| 修复 | **macOS** Cmd+Q / Dock / 退出按钮可真正结束进程（`markQuitting`） |
...
| 类别 | 内容 |
|------|------|
| 快捷访问 | 悬浮快捷搜索条（`Alt+Shift+P`）；Enter：程序 / 网址 / 定位主窗口（**v1.26.0**） |
...
│   ├── renderer/       # 主窗口、快捷条、详情窗、剪切板窗、便签窗（notes.html / note.html，v1.38.0）
...
| [docs/code-map/quickbar-and-shortcuts.md](./docs/code-map/quickbar-and-shortcuts.md) | 快捷搜索条、最近打开与全局快捷键（**v1.26.0**；**v1.43.0** 四组启动快捷键可改） |
| [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md) | **v1.32.0** 剪切板历史；**v1.33.0** 独立设置模块；**v1.34.0** 默认不固定、快捷模式；**v1.42.0** 条目标题与启动稳定性；**v1.43.0** 快捷键可改、托盘入口 |
| [docs/code-map/sticky-notes.md](./docs/code-map/sticky-notes.md) | **v1.38.0** 轻量便签；**v1.39.0** 管理窗快捷键；**v1.40.0** 分栏调宽与右键移动；**v1.41.0** 复制全部内容；**v1.42.0** 回收站预览布局修复；**v1.43.0** 桌面窗最小化、便签本右键新建、快捷键可改 |
...
| [docs/code-map/product-tour.md](./docs/code-map/product-tour.md) | **v1.24.0** 产品引导；**v1.25.0** 侧栏/列表锚点；**v1.33.0** 对齐新设置分区；**v1.39.0** 便签设置 |
...
**误删条目能恢复吗？**  
可以。删除会**移至回收站**（非立即彻底删除）。侧栏底部打开 **回收站** → 单条 **还原** 或 **一键还原**。超过 **设置 → 回收站 → 回收站保留期限**（默认 30 天）后条目会自动彻底删除。
...
**最小化后找不到窗口？**  
查看任务栏右侧系统托盘中的 PwdBook 图标，单击即可恢复窗口；右键托盘图标可打开 **设置**、**快捷搜索**、**剪切板** 或 **便签**（v1.43.0 起含后两项）。

**重复打开安装包或快捷方式？**  
应用为单实例；第二次启动会激活已有进程并提示，不会打开第二个数据目录实例。

**快捷搜索条怎么用？**  
解锁后按当前快捷键（默认 `Alt+Shift+P`，**v1.43.0** 起可在 **设置 → 悬浮条** 录制更换；也可点击「打开快捷条」）。无输入时显示**最近打开**（默认最多 5 条，可在 **设置 → 悬浮条 → 快捷条显示条数** 调至最多 20；悬停可 × 移除）；输入关键词后 ↑↓ 选择条目，`Enter`：有本地程序则启动，有网址则打开网址，均无则打开主窗口并定位该条目（**v1.26.0**）。选中条目按 `ArrowRight` 打开右侧详情；详情中用 `↑` / `↓` 选择字段、`Enter` 快捷复制，TOTP 字段显示实时动态验证码；按 `ArrowLeft` 返回悬浮条继续操作（**v1.36.0**）。列表超出可视高度时可滚动。失焦后快捷条自动隐藏。移除最近打开**不会**删除条目，也不会清除主列表的「最近使用时间」；全部移除后列表保持为空，直到再次打开条目。详见 [docs/code-map/quickbar-and-shortcuts.md](./docs/code-map/quickbar-and-shortcuts.md)。

**如何用快捷键唤起主窗口？**  
默认 `Alt+Shift+M`（**v1.43.0** 起可在 **设置 → 悬浮条** 录制更换；开关在悬浮条开关下方）。窗口最小化到托盘时也可通过该快捷键恢复。

**如何用快捷键打开便签？**  
默认 `Alt+Shift+N`（**v1.39.0**，**v1.43.0** 起可在 **设置 → 便签** 录制更换或关闭）。已打开再按一次收起；保险库锁定时先唤起主窗口以便解锁。也可从托盘右键「打开便签」进入。

**剪切板历史存在哪里？会不会同步？**  
记录只在本机渲染进程：默认 `sessionStorage`；开启「剪切板数据持久化」后写入 `localStorage`。不进 SQLite 保险库，也不随 Wi-Fi / 文件夹同步。系统监听默认关闭，需在 **设置 → 剪切板** 打开。唤出快捷键默认 `Alt+Shift+O`，**v1.43.0** 起可在同一页录制更换。**v1.34.0** 小窗可开快捷模式：回车复制后自动关窗。详见 [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md)。

**便签存在哪里？会不会同步？**  
便签写在保险库 SQLite 里，标题和正文加密，和密码条目一起备份、导入导出，并随 Wi-Fi / 文件夹同步。桌面窗口的位置、置顶和是否显示只留本机，另一台设备不会自动弹出。锁定时窗口全部隐藏。详见 [docs/code-map/sticky-notes.md](./docs/code-map/sticky-notes.md)。
