...
![Version](https://img.shields.io/badge/version-1.39.0-blue?style=flat-square)
...
| 项 | 说明 |
|----|------|
| **剪切板历史** | 开关（默认关闭）；开启后自动捕获复制的文本与图片。快捷键 `Alt+Shift+O` |
...
| 项 | 说明 |
|----|------|
| 快捷键打开便签 | 开关（默认开启）；默认 `Alt+Shift+N` 打开或收起便签管理窗口。锁定时先唤起主窗口以便解锁 |
...
| 项 | 说明 |
|----|------|
| 快捷搜索条 | 开关 +「打开快捷条」；默认 `Alt+Shift+P`；显示条数 5–20（默认 5） |
...
从侧栏底部 **工具箱 → 剪切板**、标题栏剪切板按钮，或全局快捷键 **`Alt+Shift+O`** 打开独立小窗口（须已解锁；锁定态会先唤起主窗口）。
...
- 便签本右键重命名或删除；删除时可把便签移到默认本或一并进回收站
...
- **桌面便签**（**v1.38.0**） — 从管理窗弹出；无系统边框，可置顶；关闭只隐藏；锁定保险库时全部隐藏，解锁后恢复。管理窗快捷键 **`Alt+Shift+N`**（**v1.39.0**，可在设置关闭）。见 [便签](#便签-v1380)
- **快捷键唤起主窗口** — 全局快捷键（默认 `Alt+Shift+M`）显示并聚焦主窗口；可在 **设置 → 悬浮条** 启用/禁用
...
| [docs/code-map/sticky-notes.md](./docs/code-map/sticky-notes.md) | **v1.38.0** 轻量便签；**v1.39.0** 管理窗快捷键与设置 → 便签 |
...
**快捷搜索条怎么用？**  
解锁后按 `Alt+Shift+P`（或在 **设置 → 悬浮条** 点击「打开快捷条」）。无输入时显示**最近打开**（默认最多 5 条，可在 **设置 → 悬浮条 → 快捷条显示条数** 调至最多 20；悬停可 × 移除）；输入关键词后 ↑↓ 选择条目，`Enter`：有本地程序则启动，有网址则打开网址，均无则打开主窗口并定位该条目（**v1.26.0**）。选中条目按 `ArrowRight` 打开右侧详情；详情中用 `↑` / `↓` 选择字段、`Enter` 快捷复制，TOTP 字段显示实时动态验证码；按 `ArrowLeft` 返回悬浮条继续操作（**v1.36.0**）。列表超出可视高度时可滚动。失焦后快捷条自动隐藏。移除最近打开**不会**删除条目，也不会清除主列表的「最近使用时间」；全部移除后列表保持为空，直到再次打开条目。详见 [docs/code-map/quickbar-and-shortcuts.md](./docs/code-map/quickbar-and-shortcuts.md)。

**如何用快捷键唤起主窗口？**  
默认 `Alt+Shift+M`（可在 **设置 → 悬浮条 → 快捷键唤起主窗口** 开关）。窗口最小化到托盘时也可通过该快捷键恢复。

**如何用快捷键打开便签？**  
默认 `Alt+Shift+N`（**v1.39.0**，可在 **设置 → 便签** 开关）。已打开再按一次收起；保险库锁定时先唤起主窗口以便解锁。

**剪切板历史存在哪里？会不会同步？**  
记录只在本机渲染进程：默认 `sessionStorage`；开启「剪切板数据持久化」后写入 `localStorage`。不进 SQLite 保险库，也不随 Wi-Fi / 文件夹同步。系统监听默认关闭，需在 **设置 → 剪切板** 打开。快捷键 `Alt+Shift+O`。**v1.34.0** 小窗可开快捷模式：回车复制后自动关窗。详见 [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md)。
