...
应用提供「数字保险库」与 **Animal Island** 两套视觉皮肤，标题栏可**快捷换肤**；**v1.20.0** 主窗口标题栏支持 **窗口置顶** 图钉；**v1.21.0** **设置 → 安全** 支持 **开机自动启动**（默认关闭；**v1.23.0** 修复 Windows 含空格安装路径）；**v1.22.0** 条目支持 **附件**、**自定义字段** 与 **列表/方块** 布局切换；**v1.23.0** 托盘菜单可直达 **设置**、数据库损坏时自动隔离备份并创建空库；**v1.24.0** 标题栏 **产品学习** 提供 6 条交互式引导路径；**v1.25.0** 侧栏底部改为 **工具箱 / 管理 / 回收站 / 设置** 图标栏，**标签筛选** 迁至列表工具栏；**v1.26.0** 快捷条可配置显示条数、网站条目定位主窗口、列表可滚动，并修复 macOS 无法退出；支持中英文界面；默认窗口 1200×760，自定义标题栏。官方打包支持 **Windows**（NSIS）与 **macOS**（DMG，x64 / arm64）。
...
- **悬浮快捷搜索条** — 置顶细长搜索窗（默认 `Alt+Shift+P`）；`↑`/`↓` 选择、`Enter`：**本地程序**直接启动，**仅网址**则打开主窗口并定位条目（**v1.26.0**）；无输入时显示 **最近打开**（条数可配，默认 5、最大 20，独立存储，清空后不会从历史 `last_used_at` 自动补满；**v1.26.0** 结果区固定高度可滚动）；锁定状态下唤起会引导先解锁主窗口
...
| 类别 | 内容 |
|------|------|
| 快捷条 | 网站条目 **打开主窗口并定位**；显示条数可配（5–20）；结果列表固定高度滚动 |
...
| 类别 | 内容 |
|------|------|
| 快捷访问 | 悬浮快捷搜索条（`Alt+Shift+P`）；Enter 启动程序或定位主窗口（**v1.26.0**） |
...
**快捷搜索条怎么用？**  
解锁后按 `Alt+Shift+P`（或在设置中点击「打开快捷条」）。无输入时显示**最近打开**（默认最多 5 条，可在 **设置 → 安全 → 快捷条显示条数** 调至最多 20；悬停可 × 移除）；输入关键词后 ↑↓ 选择条目，`Enter`：有本地程序则启动程序，否则打开主窗口并定位该条目（**v1.26.0**）。列表超出可视高度时可滚动。失焦后快捷条自动隐藏。移除最近打开**不会**删除条目，也不会清除主列表的「最近使用时间」；全部移除后列表保持为空，直到再次打开条目。详见 [docs/code-map/quickbar-and-shortcuts.md](./docs/code-map/quickbar-and-shortcuts.md)。
