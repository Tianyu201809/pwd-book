...
3. **预览确认** — 分 tab 查看 **将导入** / **重复跳过** / **无效** 及原因；CSV 按来源解析并**自动归入对应分类**；JSON 导入时**自动创建备份中缺失的分类**
...
**能否多端同步？**  
当前版本不支持内置云同步。可自行通过「设置 → 数据 → 导出数据」导出 **JSON**（推荐，可完整导入），或导出 CSV / Excel 作查阅，再在其他机器用「导入数据」恢复；或使用「邮箱备份」将加密 ZIP 发到自己的邮箱作灾备。
