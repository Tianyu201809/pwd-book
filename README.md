...
| **剪切板历史**（**v1.32.0**） | 开关（默认关闭）；开启后自动捕获复制的文本与图片。另可设默认清理周期（30 秒 / 5 / 15 / 30 分钟 / 永久，默认 5 分钟）与是否重启后保留。快捷键 `Alt+Shift+O` |
| 关闭窗口 | 每次询问 / 默认最小化到托盘 / 直接退出；标题栏关闭对话框可「记住选择」 |
| 快捷搜索条 | 开关 +「打开快捷条」；默认全局快捷键 `Alt+Shift+P`；**v1.26.0** 可配置显示条数（5–20，默认 5） |
...
- 开启 **设置 → 安全 → 剪切板历史** 后自动捕获系统剪切板中的文本与图片；关闭后不轮询，仍可手动新建文本
- 列表 / 预览分栏：搜索、筛选、固定单条、按条设置过期、复制回系统剪切板、清空历史
- 小窗置顶且不占任务栏；未固定时失焦隐藏；可固定小窗
...
- **剪切板历史小窗口**（**v1.32.0**） — 默认 `Alt+Shift+O`；置顶、不占任务栏；未固定时失焦隐藏；锁定态唤起会引导先解锁。见 [剪切板历史](#剪切板历史-v1320)
...
| [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md) | **v1.32.0** 剪切板历史小窗口、系统同步与本地存储 |
...
**快捷搜索条怎么用？**  
解锁后按 `Alt+Shift+P`（或在设置中点击「打开快捷条」）。无输入时显示**最近打开**（默认最多 5 条，可在 **设置 → 安全 → 快捷条显示条数** 调至最多 20；悬停可 × 移除）；输入关键词后 ↑↓ 选择条目，`Enter`：有本地程序则启动，有网址则打开网址，均无则打开主窗口并定位该条目（**v1.26.0**）。列表超出可视高度时可滚动。失焦后快捷条自动隐藏。移除最近打开**不会**删除条目，也不会清除主列表的「最近使用时间」；全部移除后列表保持为空，直到再次打开条目。详见 [docs/code-map/quickbar-and-shortcuts.md](./docs/code-map/quickbar-and-shortcuts.md)。

**如何用快捷键唤起主窗口？**  
默认 `Alt+Shift+M`（可在 **设置 → 安全 → 快捷键唤起主窗口** 开关）。窗口最小化到托盘时也可通过该快捷键恢复。

**剪切板历史存在哪里？会不会同步？**  
**v1.32.0** 记录只在本机渲染进程：默认 `sessionStorage`；开启「剪切板数据持久化」后写入 `localStorage`。不进 SQLite 保险库，也不随 Wi-Fi / 文件夹同步。系统监听默认关闭，需在 **设置 → 安全 → 剪切板历史** 打开。快捷键 `Alt+Shift+O`。详见 [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md)。
