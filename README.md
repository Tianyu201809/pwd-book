...
从侧栏进入 **设置**。**v1.33.0** 起分为：**安全 / 剪切板 / 便签 / 浏览器 / 悬浮条 / 回收站 / 外观 / 数据 / 关于**。
...
从侧栏底部 **工具箱 → 便签**，或全局快捷键 **`Alt+Shift+N`** 打开独立管理窗口（须已解锁；已打开再按一次收起；锁定态会先唤起主窗口）。可在 **设置 → 便签** 关闭该快捷键。
...
- **桌面便签**（**v1.38.0**） — 从管理窗弹出；无系统边框，可置顶；关闭只隐藏；锁定保险库时全部隐藏，解锁后恢复。管理窗快捷键 **`Alt+Shift+N`**。见 [便签](#便签-v1380)
...
| [docs/code-map/sticky-notes.md](./docs/code-map/sticky-notes.md) | **v1.38.0** 轻量便签：管理窗、桌面窗、便签本、块编辑、同步 |
...
| [docs/code-map/product-tour.md](./docs/code-map/product-tour.md) | **v1.24.0** 产品引导；**v1.25.0** 侧栏/列表锚点；**v1.33.0** 对齐新设置分区 |
