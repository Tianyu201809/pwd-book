...
- 列表 / 预览分栏：搜索、筛选、固定单条、按条设置过期、复制回系统剪切板、清空历史；**v1.33.0** 可设条数上限，失焦不再自动隐藏；可开**快捷模式**，回车复制后自动关窗
- 小窗置顶且不占任务栏；用 Esc、关闭按钮或再次快捷键收起
...
- **剪切板历史小窗口**（**v1.32.0**） — 默认 `Alt+Shift+O`；置顶、不占任务栏；**v1.33.0** 失焦不再自动隐藏，用 Esc、关闭按钮或再次快捷键收起；锁定态唤起会引导先解锁。见 [剪切板历史](#剪切板历史-v1320)
...
| 剪切板 | 条数上限；使用向导；小窗失焦不隐藏，键盘选择/复制/预览；快捷模式（回车复制后关窗） |
| 引导 | 产品探索对齐新分区与剪切板入口 |
...
| [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md) | **v1.32.0** 剪切板历史；**v1.33.0** 独立设置模块、条数上限、使用向导 |
...
**剪切板历史存在哪里？会不会同步？**  
记录只在本机渲染进程：默认 `sessionStorage`；开启「剪切板数据持久化」后写入 `localStorage`。不进 SQLite 保险库，也不随 Wi-Fi / 文件夹同步。系统监听默认关闭，需在 **设置 → 剪切板** 打开。快捷键 `Alt+Shift+O`。详见 [docs/code-map/clipboard-history.md](./docs/code-map/clipboard-history.md)。
