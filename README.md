...
| **侧栏底部栏**（**v1.25.0**） | 四图标：**工具箱**（随机密码、密码健康、**v1.32.0** 剪切板）、**管理**（分类/标签）、**回收站**、**设置**；悬停显示名称，工具箱与管理弹出子菜单；锁定改由标题栏快捷入口 |
...
- 列表 / 预览分栏：搜索、筛选、固定单条、按条设置过期、复制回系统剪切板、清空历史；**v1.33.0** 可设条数上限，失焦不再自动隐藏；**v1.34.0** 可开**快捷模式**，回车复制后自动关窗
- 小窗置顶且不占任务栏；**v1.34.0** 第一次打开默认不固定；用 Esc、关闭按钮或再次快捷键收起
...
- **剪切板历史小窗口**（**v1.32.0**） — 默认 `Alt+Shift+O`；置顶、不占任务栏；**v1.33.0** 失焦不再自动隐藏；**v1.34.0** 默认不固定，可开快捷模式（回车复制后关窗）；用 Esc、关闭按钮或再次快捷键收起；锁定态唤起会引导先解锁。见 [剪切板历史](#剪切板历史-v1320)
...
│   ├── renderer/       # 主窗口、快捷条、详情窗、剪切板窗（clipboard-window.html，v1.32.0）
